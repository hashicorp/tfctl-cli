...
func runStatus(ctx context.Context, opts *StatusOpts) error {
	...
	if opts.Profile.GetToken() == "" {
		return displayAuthFailure(opts, &authFailure{reason: reasonNoToken})
	}
	...
	if err != nil {
		return displayAuthFailure(opts, classifyAuthError(err))
	}
...
func displayAuthFailure(opts *StatusOpts, f *authFailure) error {
	hostname := opts.Profile.GetHostname()
